...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        x = SimpleRNN(units, activation='relu',
            return_sequences=True, implementation=2, name=f'rnn_{i}')(x)
...
def final_model(input_dim, units, filters=200,
                        kernel_size=11,
                        conv_stride=1,
                        conv_border_mode='valid',  dropout=0.5, recur_layers=3, output_dim=29):
...
#     x = Dropout(rate=dropout)(x)
    for i in range(recur_layers):
        x = Bidirectional(GRU(units, return_sequences=True), merge_mode='concat')(x)
        x = BatchNormalization(name = f'bn_{i}')(x)
        x = Dropout(rate=dropout)(x)
